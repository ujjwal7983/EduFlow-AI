...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(ENDPOINTS.AUTH.REGISTER, formData).catch(() => null);
      ...
      if (response && response.data) {
        ...
        // Fallback for demo without backend - spoof profile to allow registration simulation into onboarding
        setTimeout(async () => {
          await login({ name: formData.name || 'Demo Scholar', email: formData.email }, 'demo-token');
          // Important: we don't pass a profile here so it correctly triggers the ProtectedRoute onboarding redirect!
          navigate('/onboarding');
        }, 1000);
      }
    } catch (err) {
